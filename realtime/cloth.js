...
export class Cloth {
  ...
  step() {
    ...
    const DT = 0.01;
    this.windForce = [
      Math.sin(this.t / 20.0),
      Math.cos(this.t / 30.0),
      Math.sin(this.t / 10.0),
    ];
    ...
    for (let i = 0; i < this.triangles.length; i++) {
      let u = this.triangles[i][0],
        v = this.triangles[i][1],
        w = this.triangles[i][2];
      let euv = [],
        euw = [],
        n = [];
    ...
    for (let i = 0; i < this.positions.length; i++) {
      ...
      vec3.scaleAndAdd(
        ...
        this.acc[i],
        this.normals[i],
        vec3.dot(this.normals[i], this.windForce)
      );
    }
